...
# regex for manifest date replacement
manifest = re.compile('\[INTERPRETERVERSION\].+?\[\]?', re.S)

# List of files to compress (with debug code removed)
compress = (
	('.build/parchment.js', 'Parchment/parchment.min.js', 'src/parchment/header.txt'),
	('.build/parchment.css', 'Parchment/parchment.min.css', 'src/parchment/header.txt'),
	...
	('.build/zvm.js', 'Parchment/zvm.min.js', 'src/ifvms.js/src/zvm/header.txt', debugzvm),
)
...
for package in compress:
	...
	# Compress!
	command = 'java -jar ../yuicompressor-2.4.2.jar --type ' + (package[1].endswith('.js') and 'js' or 'css' ) + ' .build/temp -o .build/temp'
	os.system(command)
